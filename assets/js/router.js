...
function handleAnswer(answer, question, skillId) {
  ...
  panel.innerHTML = `
    ...
    <p class="math-content">${formatMathHtml(result.error.message)}</p>
    <p class="math-content"><b>Gợi ý:</b> ${formatMathHtml(result.error.hint)}</p>
      ...
      <div class="error-list">
        ${state.errors.length ? state.errors.map((error) => `
          <article class="error-card">
            ...
            <p class="math-content">${formatMathHtml(error.message)}</p>
            <p class="math-content"><b>Gợi ý:</b> ${formatMathHtml(error.hint)}</p>
